"""
1:1 Replica & Win-Rate Maximizer for Pine Script v6: "AB Touch - TRUE FIRST TICK".
...
2. Setup Verification at C1 [1] (close[3], close[2], low[3], low[2]).
3. Trend Filter on close[1].
4. Entry ALWAYS on C0 Candle Open First Tick (open + spComp).
5. AI Agents optimize SL/TP parameters & session filters to MAXIMIZE WIN RATE.
"""
...
def run_true_first_tick_agent(
    ...
    for i in range(200, n):
        ...
        # Setup Verification at C1 [1]: close[3], close[2], low[3], low[2], high[3], high[2]
        c2 = i - 2
        c3 = i - 3
        ...
        slDist = fixed_sl if use_fixed_sl else (atr[c1] * atr_mult)
...
def main():
    print("="*95)
    print("🌲 PINE SCRIPT v6 EXACT REPLICA: AB Touch - TRUE FIRST TICK")
    print("   AI AGENTS WIN-RATE MAXIMIZATION ENGINE ON 3-YEAR GOLD M1 DATASET")
    print("   Rule: 100% Zero Repaint | Entry ALWAYS on C0 Candle Open First Tick")
    ...
    # Warmup JIT
    _ = run_true_first_tick_agent(opens[:1000], highs[:1000], lows[:1000], closes[:1000], hours[:1000], 0, False, 3.0, 1.5, 0.0, 0.0, -1, False, 0.14, 0.01)
    ...
    sl_configs = [
        ("ATR × 1.5 (Default)", False, 3.0, 1.5),
        ("ATR × 2.0", False, 3.0, 2.0),
        ("ATR × 2.5", False, 3.0, 2.5),
        ("Fixed $3.0", True, 3.0, 1.5),
        ("Fixed $2.0", True, 2.0, 1.5),
        ("Fixed $1.5", True, 1.5, 1.5),
    ]

    pdp_list = [0.0, 4.0, 5.0, 6.0]
    pwk_list = [0.0, 0.8, 1.0, 1.2]
            ...
            for pdp in pdp_list:
                for pwk in pwk_list:
                    for sess in sess_list:
                        pnls_001 = run_true_first_tick_agent(
                            opens, highs, lows, closes, hours,
                            m_code, use_fixed, fixed_sl, atr_m,
                            pdp, pwk, -1, sess, 0.14, 0.01
                        )
                        eval_res = evaluate_agent(pnls_001)
                        if eval_res["trades"] >= 20 and eval_res["net_profit"] > 0:
                            results.append({
                                "mode": mode,
                                "sl_setting": sl_desc,
    ...
    print("\n" + "="*110)
    print("🏆 TOP 20 HIGHEST WIN-RATE AI AGENTS FOR PINE SCRIPT v6: TRUE FIRST TICK")
    print("="*110)
    ...
    memory_data = {
        ...
        "total_ai_agents_evaluated": len(results),
        "champion_strategy": {
            "mode": top_champ["mode"],
